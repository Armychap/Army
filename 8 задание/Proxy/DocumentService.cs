...
namespace Proxy
{
    public class DocumentService : IDocumentService
    {
        ...
        {
            Console.WriteLine("[SERVICE] Инициализация реального сервиса документов...");
            ...
            Console.WriteLine("[SERVICE] Загружено 4 тестовых документа");
        }

        // Чтение документа
        public string ReadDocument(int docId)
        {
            Console.WriteLine($"[SERVICE] Чтение документа {docId} из БД...");
            Thread.Sleep(200);
        ...
        {
            Console.WriteLine($"[SERVICE] Сохранение документа {docId} в БД...");
            Thread.Sleep(200);

            if (_documents.ContainsKey(docId))
                _documents[docId].Content = content;
            else
                Console.WriteLine($"[SERVICE] ОШИБКА: Документ {docId} не существует");
        }

        // Удаление документа
        public void DeleteDocument(int docId)
        {
            Console.WriteLine($"[SERVICE] Удаление документа {docId} из БД...");
            Thread.Sleep(200);
        ...
        {
            Console.WriteLine($"[SERVICE] Создание нового документа '{title}' в БД...");
            Thread.Sleep(200);
        ...
        {
            Console.WriteLine($"[SERVICE] Получение списка всех документов из БД...");
            Thread.Sleep(200);
        ...
        {
            Console.WriteLine($"[SERVICE] Получение информации о документе {docId}...");
            if (_documents.ContainsKey(docId))
            {
